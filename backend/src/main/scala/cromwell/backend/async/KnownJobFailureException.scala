...
final case class WrongReturnCode(jobTag: String, returnCode: Int, stderrPath: Option[Path])
...
final case class StderrNonEmpty(jobTag: String, stderrLength: Long, stderrPath: Option[Path])
    extends KnownJobFailureException {
  override def getMessage =
    s"stderr for job $jobTag has length $stderrLength and 'failOnStderr' runtime attribute was true."
}
...
final case class RetryWithMoreMemory(jobTag: String,
  ...
  val errorKeysAsString = memoryRetryErrorKeys match {
    case None =>
      ...
      logger.error(
        s"Programmer error: found one of the `system.memory-retry-error-keys` in the `stderr` of task but " +
  ...
  override def getMessage =
    s"stderr for job `$jobTag` contained one of the `memory-retry-error-keys${errorKeysAsString}` specified in " +
...
object RuntimeAttributeValidationFailure {
  ...
  ): RuntimeAttributeValidationFailure =
    RuntimeAttributeValidationFailure(jobTag, runtimeAttributeName, runtimeAttributeValue, None)
}
...
final case class RuntimeAttributeValidationFailure private (jobTag: String,
  ...
  override def getMessage =
    s"Task $jobTag has an invalid runtime attribute $runtimeAttributeName = ${runtimeAttributeValue map {
        _.evaluateValue(Map.empty, NoIoFunctionSet)
      } getOrElse "!! NOT FOUND !!"}"
}

final case class RuntimeAttributeValidationFailures(throwables: List[RuntimeAttributeValidationFailure])
    extends KnownJobFailureException
...
final case class JobAlreadyFailedInJobStore(jobTag: String, originalErrorMessage: String)
    extends KnownJobFailureException {
