...
trait WesRouteSupport extends HttpInstrumentation {
          ...
          concat(
              ...
              get {
                ...
                post {
                  extractSubmission() { submission =>
                    wesSubmitRequest(submission.entity, isSingleSubmission = true)
                  }
                }
            },
            path("runs" / Segment) { workflowId =>
              get {
                ...
                completeCromwellResponse(
                  runLog(workflowId,
                         (w: WorkflowId) => GetSingleWorkflowMetadataAction(w, None, None, expandSubWorkflows = false),
  ...
  def wesSubmitRequest(formData: Multipart.FormData, isSingleSubmission: Boolean): Route = {
    ...
    def sendToWorkflowStore(command: WorkflowStoreActor.WorkflowStoreActorSubmitCommand,
                            warnings: Seq[String],
      ...
      onComplete(workflowStoreActor.ask(command).mapTo[WorkflowStoreSubmitActor.WorkflowStoreSubmitActorResponse]) {
        ...
        case Failure(_: AskTimeoutException) if CromwellShutdown.shutdownInProgress() =>
          respondWithWesError("Cromwell service is shutting down", StatusCodes.InternalServerError)
      ...
      case Success(data) =>
        PartialWorkflowSources.fromSubmitRoute(data, allowNoInputs = isSingleSubmission) match {
          case Success(workflowSourceFiles) if isSingleSubmission && workflowSourceFiles.size == 1 =>
            ...
            sendToWorkflowStore(WorkflowStoreActor.SubmitWorkflow(workflowSourceFiles.head),
                                warnings,
...
object WesRouteSupport {
  ...
  def WesAbortSuccessHandler: PartialFunction[SuccessfulAbortResponse, Route] = { case response =>
    complete(WesRunId(response.workflowId.toString))
  }
  ...
  def WesAbortErrorHandler: PartialFunction[Throwable, Route] = {
    ...
    case _: AskTimeoutException if CromwellShutdown.shutdownInProgress() =>
      respondWithWesError("Cromwell service is shutting down", StatusCodes.InternalServerError)
  ...
  def listRuns(pageSize: Option[Int], pageToken: Option[String], serviceRegistryActor: ActorRef): Future[WesResponse] =
    ...
    metadataQueryRequest(Seq.empty[(String, String)], serviceRegistryActor)
      .map(RunListResponse.fromMetadataQueryResponse)

  def runLog(workflowId: String,
    ...
    metadataJsonResponse.map {
      ...
      case FailedMetadataJsonResponse(_, reason) =>
        WesErrorResponse(reason.getMessage, StatusCodes.InternalServerError.intValue)
    }
  }
}