...
import cromwell.services.metadata.impl.MetadataSummaryRefreshActor.{
  MetadataSummaryFailure,
  MetadataSummarySuccess,
  SummarizeMetadata
}
...
object MetadataServiceActor {
  ...
  def props(serviceConfig: Config, globalConfig: Config, serviceRegistryActor: ActorRef) =
    Props(MetadataServiceActor(serviceConfig, globalConfig, serviceRegistryActor)).withDispatcher(ServiceDispatcher)
}
...
case class MetadataServiceActor(serviceConfig: Config, globalConfig: Config, serviceRegistryActor: ActorRef)
  ...
  override val supervisorStrategy = new OneForOneStrategy()(decider) {
    override def logFailure(context: ActorContext, child: ActorRef, cause: Throwable, decision: Directive) = {
      ...
      log.error(
        cause,
        s"The $childName Metadata Actor died unexpectedly, metadata events might have been lost. Restarting it..."
      )
    }
  }
  ...
  private val metadataTableMetricsPath: NonEmptyList[String] =
    MetadataServiceActor.MetadataInstrumentationPrefix :+ "table"
  ...
  val readActor = context.actorOf(
    ReadMetadataRegulatorActor.props(metadataBuilderActorProps, readMetadataWorkerActorProps),
    "ClassicMSA-ReadMetadataRegulatorActor"
  )
  ...
  metadataTableMetricsInterval.map(
    context.system.scheduler.schedule(1.minute, _, self, SendMetadataTableSizeMetrics)(context.dispatcher, self)
  )

  private def scheduleSummary(): Unit =
    metadataSummaryRefreshInterval foreach { interval =>
      summaryRefreshCancellable =
        Option(context.system.scheduler.scheduleOnce(interval, self, RefreshSummary)(context.dispatcher, self))
    }
  ...
  private def buildSummaryActor: Option[ActorRef] = {
    val actor = metadataSummaryRefreshInterval map { _ =>
      context.actorOf(MetadataSummaryRefreshActor.props(serviceRegistryActor), "metadata-summary-actor")
    }
  ...
  private def buildArchiveMetadataActor: Option[ActorRef] =
    if (serviceConfig.hasPath("archive-metadata")) {
      ...
      ArchiveMetadataConfig.parseConfig(serviceConfig.getConfig("archive-metadata"))(context.system) match {
        case Right(config) =>
          Option(
            context.actorOf(ArchiveMetadataSchedulerActor.props(config, serviceRegistryActor),
                            "archive-metadata-scheduler"
            )
          )
        case Left(errorList) =>
          throw AggregatedMessageException("Failed to parse the archive-metadata config", errorList.toList)
      }
    } else {
  ...
  private def buildDeleteMetadataActor: Option[ActorRef] =
    if (serviceConfig.hasPath("delete-metadata")) {
      ...
      DeleteMetadataConfig.parseConfig(serviceConfig.getConfig("delete-metadata")) match {
        case Right(config) =>
          Option(context.actorOf(DeleteMetadataActor.props(config, serviceRegistryActor), "delete-metadata-actor"))
        case Left(errorList) =>
          throw AggregatedMessageException("Failed to parse the archive-metadata config", errorList.toList)
      }
    } else {
    ...
    getMetadataArchiveStatusAndEndTime(workflowId) onComplete {
      case Success(WorkflowArchiveStatusAndEndTimestamp(status, endTime)) =>
        MetadataArchiveStatus.fromDatabaseValue(status).toTry match {
          ...
          case Failure(e) =>
            sender ! FailedToGetArchiveStatusAndEndTime(
              new RuntimeException(s"Failed to get metadata archive status for workflow ID $workflowId", e)
            )
        }
      case Failure(e) =>
        sender ! FailedToGetArchiveStatusAndEndTime(
  ...
  private def sendMetadataTableSizeMetrics(): Unit =
    getMetadataTableSizeInformation() onComplete {
      ...
      case Failure(e) =>
        log.error(e, s"Error fetching metadata table size metrics. Will try again in $metadataTableMetricsInterval...")
    }
  ...
  def receive = summarizerReceive orElse {
    case ShutdownCommand =>
      waitForActorsAndShutdown(NonEmptyList.of(writeActor) ++ archiveMetadataActor.toList ++ deleteMetadataActor.toList)
    ...
    case g: FetchWorkflowMetadataArchiveStatusAndEndTime =>
      fetchWorkflowMetadataArchiveStatusAndEndTime(g.workflowId, sender())
